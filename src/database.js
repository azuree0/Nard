...
export async function initDatabase() {
  ...
  try {
    ...
    const savedDb = localStorage.getItem('nard_database');
    if (savedDb) {
      try {
...
function createTables() {
  ...
  db.run(`
    CREATE TABLE IF NOT EXISTS games (
      ...
      player1 TEXT DEFAULT 'White',
      player2 TEXT DEFAULT 'Black',
      winner TEXT,
...
function saveDatabase() {
  ...
  try {
    ...
    localStorage.setItem('nard_database', base64);
  } catch (error) {
 ...
 */
export function createGame(player1 = 'White', player2 = 'Black') {
  if (!db) {
 ...
 */
export function saveMove(gameId, player, pointFrom, pointTo, dieValue, moveNumber) {
  if (!db || !gameId) {
    return;
  }
  ...
  try {
    db.run(
      `INSERT INTO moves (game_id, player, point_from, point_to, die_value, move_number)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [gameId, player, pointFrom, pointTo, dieValue, moveNumber]
    );
    saveDatabase();
  } catch (error) {
 ...
 * Save game state
 */
export function saveGameState(gameId, boardState, currentPlayer, diceValues) {
  if (!db || !gameId) return;
  
  try {
    const boardJson = JSON.stringify(boardState);
    const diceJson = JSON.stringify(diceValues);
    db.run(
      `INSERT INTO game_states (game_id, board_state, current_player, dice_values)
       VALUES (?, ?, ?, ?)`,
      [gameId, boardJson, currentPlayer, diceJson]
    );
    saveDatabase();
  } catch (error) {
...
export function getGameStats(games = null) {
  if (!db) return { totalGames: 0, whiteWins: 0, blackWins: 0, totalMoves: 0 };
  
  try {
    const gamesList = games || getAllGames();
    const totalGames = gamesList.length;
    const whiteWins = gamesList.filter(g => g.winner === 'White').length;
    const blackWins = gamesList.filter(g => g.winner === 'Black').length;
    ...
    return { totalGames: 0, whiteWins: 0, blackWins: 0, totalMoves: 0 };
  }
}
